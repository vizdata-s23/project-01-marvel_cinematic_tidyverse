...
p <- ggplot(comprehensive_heat, aes(x=maxplayers, y = minplayers, fill = average)) +
  ...
  labs(title = "Average Rating Based on Min and Max Players", x = "Minimum Number of Players", y = "Maximum Number of Players", fill = "Average Rating") + theme_grey() + guides(fill = guide_colourbar(barwidth = 0.5, barheight = 8)) + coord_cartesian(ylim=c(0,5), xlim = c(0, 12))
...
```{r}
comprehensive_plot3 <- comprehensive |>
...
comprehensive_plot4 |>
  ggplot(aes(x = factor(expansions), fill = factor(expansions))) +
  geom_bar() +
  scale_x_discrete(limits = c("1", "2", "3", "4", "5", "6")) +
  geom_hline(yintercept = 100, color = "red") + labs(title = "Board Game Expansions vs Number Owned", x = "Number of Expansions", y = "Number Owned")+
  theme(legend.position="none")

comprehensive_plot4 |>
  filter(expansions < 7) |>
  ggplot(aes(x = factor(expansions), y = log(owned))) + 
  geom_boxplot() + labs(title = "Board Game Expansions vs Number Owned", x = "log(Number of Expansions)", y = "log(Number Owned)")
```
